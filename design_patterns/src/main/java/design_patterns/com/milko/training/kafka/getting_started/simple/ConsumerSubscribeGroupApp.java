...
public class ConsumerSubscribeGroupApp {
    ...
    public static void main(String[] args){
        ...
        props.put("group.id", "my-group"); // Required when subscribing to topics. Required for consumer group
        props.put("heartbeat.interval.ms", 3000);
        props.put("max.partition.fetch.bytes", 1048576);
        props.put("session.timeout.ms", 30000);
        props.put("connections.max.idle.ms", 540000);
        props.put("enable.auto.commit", true); //give responsibility of Kafka of current position offset to be commited offsets
        props.put("auto.commit.interval.ms", 5000); // auto commit offset interval
        props.put("auto.offset.reset", "latest"); //start read from new partition - latest(default), earliest, none
        ...
        props.put("retry.backoff.ms", 100); // when auto-commit is off and commit  type is synchronous  retry until succeeds  or unrecoverable error
        props.put("client.id", "");
        ...
        topics.add("replicated_topic_1");
        topics.add("replicated_topic_2");
