...
//        props.put("fetch.min.bytes", 1); //fetch minum bytes which is returned from poll
//        props.put("fetch.max.wait.ms", 500); //how much time to wait if above threshold is not met
//        props.put("max.partition.fetch.bytes", 1048576); //limit maximum number of bytes in poll per partition
//        props.put("max.poll.records", 2147483647); // maximum number of records in poll
//        props.put("group.id", "");
...
//        props.put("client.id", "");
        ...
        partitions.add(new TopicPartition("replicated_topic_1", 2)); // Adds a TopicPartition instance representing a topic and a partition.
        partitions.add(new TopicPartition("replicated_topic_2", 0)); // Adds an additional TopicPartition instance representing a different partition within the topic. Change as desired.
